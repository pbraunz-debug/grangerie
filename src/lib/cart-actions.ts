...
export async function getCartAction(): Promise<Cart> {
  if (!shopifyConfigured) return demoToCart(await readDemoCart())
  const jar = await cookies()
  const cartId = jar.get(CART_ID_COOKIE)?.value
  if (!cartId) return withGift({lines: [], subtotal: 0})
  try {
    const cart = await getCart(cartId)
    return cart ? withGift(cart) : withGift({lines: [], subtotal: 0})
  } catch {
    return withGift({lines: [], subtotal: 0})
  }
}
